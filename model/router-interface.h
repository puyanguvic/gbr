...
class LinkRecord
{
  public:
     ...
     * @see SetLinkData
     */
    LinkRecord(LinkType linkType, Ipv4Address linkId, Ipv4Address linkData, uint16_t metric);
...
class Router : public Object
{
  public:
     ...
     *
     */
    RoutingTableEntry* GetInjectedRoute(uint32_t i);
  ...
  private:
    ...
    Ptr<GBR> m_routingProtocol; //!< the Ipv4GlobalRouting in use

    typedef std::list<RoutingTableEntry*> InjectedRoutes; //!< container of Ipv4RoutingTableEntry
    typedef std::list<RoutingTableEntry*>::const_iterator
        InjectedRoutesCI; //!< Const Iterator to container of Ipv4RoutingTableEntry
    typedef std::list<RoutingTableEntry*>::iterator
     ...
     */
    Router& operator=(Router& sr);
};

} // namespace ns3

#endif /* ROUTER_INTERFACE_H */
