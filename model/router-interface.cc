...
LinkRecord::LinkRecord(LinkType linkType, Ipv4Address linkId, Ipv4Address linkData, uint16_t metric)
    : m_linkId(linkId),
      m_linkData(linkData),
      m_linkType(linkType),
      m_metric(metric)
...
LSA::LSA(LSA::SPFStatus status, Ipv4Address linkStateId, Ipv4Address advertisingRtr)
    : m_lsType(LSA::Unknown),
      ...
      m_node_id(lsa.m_node_id)
{
    ...
    NS_ASSERT_MSG(IsEmpty(), "LSA::LSA (): Non-empty LSA in constructor");
    CopyLinkRecords(lsa);
}
...
TypeId
Router::GetTypeId(void)
{
    static TypeId tid = TypeId("ns3::Router").SetParent<Object>().SetGroupName("dgr");
    return tid;
}
...
void
Router::SetRoutingProtocol(Ptr<GBR> routing)
{
...
Ptr<GBR>
Router::GetRoutingProtocol(void)
{
    NS_LOG_FUNCTION(this);
...
//
// DiscoverLSAs is called on all nodes in the system that have a Router
...
Router::DiscoverLSAs()
{
    ...
    NS_ABORT_MSG_UNLESS(node, "Router::DiscoverLSAs (): GetObject for <Node> interface failed");
    ...
    NS_ABORT_MSG_UNLESS(ipv4Local,
                        "Router::DiscoverLSAs (): GetObject for <Ipv4> interface failed");
    ...
    for (uint32_t i = 0; i < numDevices; ++i)
    {
        ...
        if (NetDeviceIsBridged(ndLocal))
        {
            ...
            NS_ABORT_MSG_IF(
                rc,
                "Router::DiscoverLSAs(): Bridge ports must not have an IPv4 interface index");
        }
        ...
        {
            NS_ASSERT_MSG(0, "Router::DiscoverLSAs (): unknown link type");
        }
    }
...
void
Router::ProcessBroadcastLink(Ptr<NetDevice> nd, LSA* pLSA, NetDeviceContainer& c)
{
...
void
Router::ProcessSingleBroadcastLink(Ptr<NetDevice> nd, LSA* pLSA, NetDeviceContainer& c)
{
    NS_LOG_FUNCTION(this << nd << pLSA << &c);

    LinkRecord* plr = new LinkRecord;
    NS_ABORT_MSG_IF(plr == 0, "Router::ProcessSingleBroadcastLink(): Can't alloc link record");
    ...
    NS_ABORT_MSG_UNLESS(
        ipv4Local,
        "Router::ProcessSingleBroadcastLink (): GetObject for <Ipv4> interface failed");
    ...
    NS_ABORT_MSG_IF(
        rc == false,
        "Router::ProcessSingleBroadcastLink(): No interface index associated with device");
    ...
    else
    {
        ...
        if (desigRtr != "255.255.255.255")
        {
            ...
            NS_ABORT_MSG_UNLESS(networkHere == networkThere,
                                "Router::ProcessSingleBroadcastLink(): Network number confusion ("
...
void
Router::ProcessBridgedBroadcastLink(Ptr<NetDevice> nd, LSA* pLSA, NetDeviceContainer& c)
{
    NS_LOG_FUNCTION(this << nd << pLSA << &c);
    NS_ASSERT_MSG(nd->IsBridge(),
                  "Router::ProcessBridgedBroadcastLink(): Called with non-bridge net device");
  ...
  NS_ABORT_MSG_UNLESS (bnd, "Router::DiscoverLSAs (): GetObject for <BridgeNetDevice> failed");
  ...
  Ptr<Node> node = nd->GetNode ();
  Ptr<Ipv4> ipv4Local = node->GetObject<Ipv4> ();
  NS_ABORT_MSG_UNLESS (ipv4Local, "Router::ProcessBridgedBroadcastLink (): GetObject for <Ipv4> interface failed");

  // Initialize to value out of bounds to silence compiler
  uint32_t interfaceLocal = ipv4Local->GetNInterfaces () + 1;
  bool rc = FindInterfaceForDevice (node, nd, interfaceLocal);
  NS_ABORT_MSG_IF (rc == false, "Router::ProcessBridgedBroadcastLink(): No interface index associated with device");
  ...
  LinkRecord *plr = new LinkRecord;
  NS_ABORT_MSG_IF (plr == 0, "Router::ProcessBridgedBroadcastLink(): Can't alloc link record");
...
void
Router::ProcessPointToPointLink(Ptr<NetDevice> ndLocal, LSA* pLSA)
{
    ...
    NS_ABORT_MSG_UNLESS(
        ipv4Local,
        "Router::ProcessPointToPointLink (): GetObject for <Ipv4> interface failed");

    uint32_t interfaceLocal = ipv4Local->GetNInterfaces() + 1;
    bool rc = FindInterfaceForDevice(nodeLocal, ndLocal, interfaceLocal);
    NS_ABORT_MSG_IF(
        rc == false,
        "Router::ProcessPointToPointLink (): No interface index associated with device");
    ...
    NS_ABORT_MSG_UNLESS(ipv4Remote,
                        "Router::ProcessPointToPointLink(): GetObject for remote <Ipv4> failed");

    //
    // Further note the requirement that nodes on either side of a point-to-point
    // link must participate in global routing and therefore have a Router
    // interface aggregated.
    //
    Ptr<Router> rtrRemote = nodeRemote->GetObject<Router>();
    if (!rtrRemote)
    {
    ...
    NS_ABORT_MSG_IF(
        rc == false,
        "Router::ProcessPointToPointLinks(): No interface index associated with remote device");
    ...
    if (ipv4Remote->IsUp(interfaceRemote))
    {
        ...
        plr = new LinkRecord;
        NS_ABORT_MSG_IF(plr == 0, "Router::ProcessPointToPointLink(): Can't alloc link record");
...
void
Router::BuildNetworkLSAs(NetDeviceContainer c)
{
    ...
    for (uint32_t i = 0; i < nDesignatedRouters; ++i)
    {
        ...
        NS_ABORT_MSG_IF(rc == false,
                        "Router::BuildNetworkLSAs (): No interface index associated with device");
        ...
        LSA* pLSA = new LSA;
        NS_ABORT_MSG_IF(pLSA == 0, "Router::BuildNetworkLSAs(): Can't alloc link record");
        ...
        // and, if we find a node with a Router interface and an IPv4
        ...
        for (uint32_t i = 0; i < deviceList.GetN(); i++)
        {
            ...
            // Does the node in question have a Router interface?  If not it can
            // hardly be considered an attached router.
            //
            Ptr<Router> rtr = tempNode->GetObject<Router>();
            if (!rtr)
            {
                NS_LOG_LOGIC("Node " << tempNode->GetId()
                                     << " does not have Router interface--skipping");
                continue;
            }
...
NetDeviceContainer
Router::FindAllNonBridgedDevicesOnLink(Ptr<Channel> ch) const
{
...
// attached and look for nodes with Router interfaces on them (one of them
...
//
Ipv4Address
Router::FindDesignatedRouterForLink(Ptr<NetDevice> ndLocal) const
{
    ...
    for (uint32_t i = 0; i < nDevices; i++)
    {
        ...
        if (bnd)
        {
            ...
            //
            // We require a designated router to have a Router interface and
            ...
            Ptr<Router> rtr = nodeOther->GetObject<Router>();
...
// that has a Router interface aggregated.  Life gets more complicated
// when there is a bridged net device on the other side.
//
bool
Router::AnotherRouterOnLink(Ptr<NetDevice> nd) const
{
    ...
    for (uint32_t i = 0; i < nDevices; i++)
    {
        ...
        Ptr<Router> rtr = nodeTemp->GetObject<Router>();
        if (rtr)
        {
            NS_LOG_LOGIC("Found Router interface, return true");
            return true;
        }
        else
        {
            NS_LOG_LOGIC("No Router interface on device, continue search");
        }
    }
...
//
bool
Router::GetLSA(uint32_t n, LSA& lsa) const
{
    NS_LOG_FUNCTION(this << n << &lsa);
    NS_ASSERT_MSG(lsa.IsEmpty(), "Router::GetLSA (): Must pass empty LSA");
    //
...
void
Router::InjectRoute(Ipv4Address network, Ipv4Mask networkMask)
{
    ...
    //
    *route = RoutingTableEntry::CreateNetworkRouteTo(network, networkMask, 1);
    m_injectedRoutes.push_back(route);
}

RoutingTableEntry*
Router::GetInjectedRoute(uint32_t index)
{
...
void
Router::RemoveInjectedRoute(uint32_t index)
{
    NS_LOG_FUNCTION(this << index);
...
bool
Router::WithdrawRoute(Ipv4Address network, Ipv4Mask networkMask)
{
    NS_LOG_FUNCTION(this << network << networkMask);
    for (InjectedRoutesI i = m_injectedRoutes.begin(); i != m_injectedRoutes.end(); i++)
    {
...
//
Ptr<NetDevice>
Router::GetAdjacent(Ptr<NetDevice> nd, Ptr<Channel> ch) const
{
    NS_LOG_FUNCTION(this << nd << ch);
    NS_ASSERT_MSG(ch->GetNDevices() == 2,
                  "Router::GetAdjacent (): Channel with other than two devices");
    //
    ...
    {
        NS_ASSERT_MSG(false, "Router::GetAdjacent (): Wrong or confused channel?");
        return 0;
    }
}
...
//
bool
Router::FindInterfaceForDevice(Ptr<Node> node, Ptr<NetDevice> nd, uint32_t& index) const
{
...
//
Ptr<BridgeNetDevice>
Router::NetDeviceIsBridged(Ptr<NetDevice> nd) const
{
    NS_LOG_FUNCTION(this << nd);
    ...
    for (uint32_t i = 0; i < nDevices; ++i)
    {
        ...
        if (ndTest->IsBridge())
        {
            ...
            NS_ABORT_MSG_UNLESS(bnd,
                                "Router::DiscoverLSAs (): GetObject for <BridgeNetDevice> failed");
...
//
void
Router::ClearBridgesVisited(void) const
{
...
//
bool
Router::BridgeHasAlreadyBeenVisited(Ptr<BridgeNetDevice> bridgeNetDevice) const
{
...
//
void
Router::MarkBridgeAsVisited(Ptr<BridgeNetDevice> bridgeNetDevice) const
{
