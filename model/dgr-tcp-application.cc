...
void
DGRTcpApplication::Setup(Ptr<Socket> socket,
                         Address sinkAddress,
                         uint64_t maxBytes,
                         uint32_t budget,
                         bool flag)
...
// Application Methods
void
DGRTcpApplication::StartApplication(void) // Called at time specified by Start
{
    NS_LOG_FUNCTION(this);
    Address from;
    ...
    if (!m_socket)
    {
        ...
        m_socket->SetConnectCallback(MakeCallback(&DGRTcpApplication::ConnectionSucceeded, this),
                                     MakeCallback(&DGRTcpApplication::ConnectionFailed, this));
        m_socket->SetSendCallback(MakeCallback(&DGRTcpApplication::DataSend, this));
    }
    if (m_connected)
    {
...
void
DGRTcpApplication::StopApplication(void) // Called at time specified by Stop
{
    NS_LOG_FUNCTION(this);
...
void
DGRTcpApplication::SendData(const Address& from, const Address& to)
{
    NS_LOG_FUNCTION(this);
...
void
DGRTcpApplication::ConnectionSucceeded(Ptr<Socket> socket)
{
...
void
DGRTcpApplication::ConnectionFailed(Ptr<Socket> socket)
{
    NS_LOG_FUNCTION(this << socket);
...
void
DGRTcpApplication::DataSend(Ptr<Socket> socket, uint32_t)
{
    NS_LOG_FUNCTION(this);
